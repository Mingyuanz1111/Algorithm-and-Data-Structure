...
* มีอีกวิธีในการเขียน DFS คือการใช้โครงสร้างข้อมูล stack มาช่วยในการจัดลำดับการสำรวจ เนื่องจากโครงสร้างนี้มีลำดับเหมือนกับการ recursive แต่มันไม่สะดวกต่อการเขียนเท่าการ recursive วิธีนี้มักใช้แทนการ recursive เฉพาะตอนที่ใช้ recursive แล้ว memory limit exceeded  
...
* หลังจากนั้นจะใช้ while loop เพื่อทำขั้นตอนเรียงตามนี้ในการ loop แต่ละครั้ง :
    ...
    3) push แต่ละ vertex ที่อยู่ติดกับ `cur` และยังไม่เคยถูกเยี่ยม เข้าไปใน stack เพื่อรอทำการเยี่ยมใน loop ครั้งต่อๆไป
...
* ที่ทำเช่นนี้แล้วสามารถสำรวจทีละความลึกของกราฟได้ เพราะว่า  
    * ตอนแรก queue มีเพียงแค่ vertex ที่ความลึกเท่ากับ 0 ซึ่งก็คือ vertex เริ่มต้น
...
> Code: ตัวอย่างการนำเข้าข้อมูลของกราฟ และสำรวจโดยใช้ BFS โดยเริ่มจาก vertex ที่ 0
```c++
#define MAXN 200001
vector<int> graph[MAXN]; // adjacency list ของกราฟที่จะสำรวจ
bool pushed[MAXN]; // มีค่าเป็น false ทุกช่อง
...
int main(){
    ...
    pushed[src]=true; // จดว่า cur ถูกเพิ่มใส่ queue แล้ว
    ...
    while(!que.empty()){
        ...
        // loop ทุก vertex รอบๆ cur ที่ยังไม่ถูกเพิ่มเข้า queue แล้วทำการเพิ่มใส่ queue ต่อ
        for(auto v: graph[cur]) if(!pushed[v]){
            que.push(v);
            pushed[v]=true; // จดว่า ถูกเพิ่มใส่ queue ไปแล้ว
        }
    }
...
* หลังจากนั้นจะใช้ while loop เพื่อทำขั้นตอนเรียงตามนี้ในการ loop แต่ละครั้ง :
    1) นำเลข vertex ที่อยู่หน้าสุดของ queue มาใส่ตัวแปร `cur` เพื่อความง่ายต่อการใช้งาน
    2) pop เลข vertex ที่อยู่หน้าสุดของ queue ออก เนื่องจากกำลังจะสำรวจ vertex นี้แล้ว
    3) push เลขของแต่ละ vertex ที่อยู่ติดกับ `cur` และยังไม่เคยถูกเพิ่มใส่ queue เข้าไปใน queue และ**จด vertex เหล่านี้ว่าถูกเพิ่มใส่ใน queue แล้ว ทันทีหลังจากเพิ่มเข้า queue**  

* loop จะจบลงเมื่อ queue ว่าง เพราะมันหมายความว่าไม่มี vertex ใดให้สำรวจต่อแล้ว
* เมื่อมีการ loop ต่อไปเรื่อยๆ จะมี vertex ในกราฟที่ถูก push ใส่ queue แล้วมากขึ้นเรื่อยๆ จนสุดท้าย loop จะทำงานเสร็จเมื่อทุก vertex ที่ไปหาได้จากจุดเริ่มต้นนั้นถูก push ใส่ queue หมดแล้ว
* สังเกตว่าเมื่อจด vertex ที่ถูกเพิ่มใส่ queue ไว้ ด้วย array `pushed` ก่อนที่จะสำรวจต่อไป จะทำให้ระหว่างสำรวจต่อ จะไม่มีการเพิ่ม vertex เดิมเข้าไปใน queue ซ้ำอีกแล้ว
...
<img src="https://github.com/user-attachments/assets/ae72d19b-ff16-436e-8c18-b940bc36e85d" width="800">
...
```c++
for(int src=0; src<n; src++) if(!pushed[src]){
